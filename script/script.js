  ...
  init() {
    for (let i = 0; i < this.plusButton.length; i++) {
      ...
      this.deleteButton[i].addEventListener('click', () => {
        ...
        let articlePrice = parseInt(this.price[i].innerHTML) * parseInt(this.qty[i].innerHTML);
        let currentTotal = parseInt(this.sum.value);
        this.sum.value = currentTotal - articlePrice;
        this.calculateTotal();
      });
    }
...
document.addEventListener('DOMContentLoaded', () => {
  new ShoppingCart();
        ...
        let articlePrice = parseInt(this.price[i].innerHTML) * parseInt(this.qty[i].value);
